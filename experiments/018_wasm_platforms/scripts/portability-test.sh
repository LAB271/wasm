...
echo ""
echo "  What the blocked legs establish, which the passing ones cannot:"
echo ""
echo "    * One artifact covers wasmtime, Spin and wasmCloud. It does NOT cover"
echo "      'WASM platforms' in general — legs 1-3 alone would overstate that."
echo "    * The two blocks differ in kind, and only one is bridgeable. Cloudflare's"
echo "      is a container format (leg 6 crosses it). Fastly's is a missing"
echo "      interface, and leg 7 shows why: it implements its own ABI instead."
echo "      wasi-http has not been converged on; it is one bet among several."
echo "    * Budget consequence: targeting Fastly as well means a second build,"
echo "      not a second config."
echo ""
